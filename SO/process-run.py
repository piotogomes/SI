...
parser = OptionParser()
parser.add_option('-s', '--seed', default=0, help='a semente do gerador de números aleatórios', action='store', type='int', dest='seed')
parser.add_option('-l', '--processlist', default='',
                  help='uma lista com elementos separados por vírgula, no seguinte formato X1:Y1,X2,Y2,... na qual X é o número de instruções que o processo deve executar e Y é a probabilidade (de 0 a 100) da instrução ser de uso puro de CPU (isto é, não inicia uma operação de E/S).',
                  action='store', type='string', dest='process_list')
parser.add_option('-L', '--iolength', default=5, help='duração de uma operação de E/S', action='store', type='int', dest='io_length')
parser.add_option('-S', '--switch', default='SWITCH_ON_IO',
                  help='define política do instante de troca entre processos, escolha entre: SWITCH_ON_IO, SWITCH_ON_END',
                  action='store', type='string', dest='process_switch_behavior')
parser.add_option('-I', '--iodone', default='IO_RUN_LATER',
                  help='define comportamento ao terminar operação de E/S, escolha entre: IO_RUN_LATER, IO_RUN_IMMEDIATE',
                  action='store', type='string', dest='io_done_behavior')
parser.add_option('-c', help='computar respostas', action='store_true', default=False, dest='solve')
parser.add_option('-p', '--printstats', help='calcula e mostra estatísticas; útil apenas com a flag -c (caso contrário as estatísticas não são impressas)', action='store_true', default=False, dest='print_stats')
...
if options.solve == False:
    print ('Qual é a sequência de eventos que a execução dos processos abaixo gera?')
    print ()
    # Produce a trace of what would happen when you run these processes:
    for pid in range(s.get_num_processes()):
        print ('Processo %d' % pid)
        for inst in range(s.get_num_instructions(pid)):
            print ('  %s' % s.get_instruction(pid, inst))
        print ('')
    print ('Leve em conta as seguintes políticas:')
    print ('  O sistema troca de processo quando', end=' ')
    if options.process_switch_behavior == SCHED_SWITCH_ON_IO:
        print ('o processo atual TERMINA ou INICIA E/S')
    else:
        print ('o processo atual TERMINA')
    print ('  Após finalizar operações de E/S, o processo que iniciou a operação irá', end=' ')
    if options.io_done_behavior == IO_RUN_IMMEDIATE:
        print ('executar IMEDIATAMENTE')
    else:
        print ('executar OPORTUNAMENTE (quando chegar a sua vez)')
    print ('')
    exit(0)
...
if options.print_stats:
    print ('')
    print ('Stats: Tempo Total %d' % clock_tick)
    print ('Stats: Uso da CPU %d (%.2f%%)' % (cpu_busy, 100.0 * float(cpu_busy)/clock_tick))
    print ('Stats: Uso de I/O %d (%.2f%%)' % (io_busy, 100.0 * float(io_busy)/clock_tick))
    print ('')
