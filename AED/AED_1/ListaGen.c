...
bool LG_comparar(NOListaGen *p1, NOListaGen *p2)
{
    if (!p1 && !p2)
        return true;
    if ((p1 && !p2) || (!p1 && p2) || (p1->tipo != p2->tipo))
        return false;
    if (p1->tipo == 1 && p1->chave != p2->chave)
        return false;
    if (p1->tipo == 2)
    {
...
NOListaGen *LG_busca(NOListaGen *p, int ch, NOListaGen **ant, bool *achou)
{
    if (!p)
        return NULL;
    if (p->tipo == 1 && p->chave == ch)
    {
        *achou = true;
        return p;
    }
    ...
    {
        *ant = p;
        return LG_busca(p->subLista, ch, ant, achou);
    }
    if (!(*achou))
    {
        *ant = p;
        return LG_busca(p->prox, ch, ant, achou);
    }
}

void LG_inserir(NOListaGen* p, int tipo, int ch)
{
    NOListaGen* atual = LG_busca()
    if (!atual)
        return;
    NOListaGen *novo = (NOListaGen *)malloc(sizeof(NOListaGen));
    novo->tipo = tipo;
